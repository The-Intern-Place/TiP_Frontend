...
const Offer = () => {
  const { ITEMS_PER_PAGE, paginate, currentPage, currentoffersdata } =
        ...
        <tbody>
          {currentoffersdata.map((t) => (
            <tr key={t.id} className="border-[1px] border-b">
              ...
              <td
                className={`py-2 h-[34px] px-4  border-[0px] rounded-[50%] text-[14px] leading-[22px] font-[600] font-epilogue    text-[#0046BF] w-[77px]  }`}
              >
                <h1 className="border-[#0046BF] border-[1px] text-[#0046BF] w-[77px]  h-[34px] rounded-[80px]  text-center flex items-center justify-center">
                  {t.statusOffered}
                </h1>
              </td>
            </tr>
