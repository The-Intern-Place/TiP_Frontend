...
const Review = () => {
  const { currentreviewdata, ITEMS_PER_PAGE, paginate, currentPage } =
        ...
        <tbody>
          {currentreviewdata.map((t) => (
            <tr key={t.id} className="border-[1px] border-b">
              ...
              <td
                className={`py-2 h-[34px] px-4  border-[0px] rounded-[50%] text-[14px] leading-[22px] font-[600] font-epilogue    text-[#FFB836] w-[107px]  }`}
              >
                <h1 className="border-[#FFB836] border-[1px] text-[#FFB836] w-[107px]  h-[34px] rounded-[80px]  text-center flex items-center justify-center">
                  {t.statusReview}
                </h1>
              </td>
            </tr>
