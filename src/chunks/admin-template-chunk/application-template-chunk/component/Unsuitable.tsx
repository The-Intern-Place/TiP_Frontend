...
import { unsuitabledata } from "@/utils/data/dashboardapplication";

const Unsuitable = () => {
  const { ITEMS_PER_PAGE, paginate, currentPage, currentunsuitabledata } =
  ...
  return (
    <div className="overflow-x-auto flex flex-col bg-[#FAFBFC]">
        ...
        <tbody>
          {currentunsuitabledata.map((t) => (
            <tr key={t.id} className="border-[1px] border-b">
              ...
              <td
                className={`py-2 h-[34px] px-4  border-[0px] rounded-[50%] text-[14px] leading-[22px] font-[600] font-epilogue    text-[#FF6550] w-[107px]  }`}
              >
                <h1 className="border-[#FF6550] border-[1px] text-[#FF6550] w-[107px]  h-[34px] rounded-[80px]  text-center flex items-center justify-center">
                  {t.statusSuitable}
                </h1>
              </td>
            </tr>
      ...
      <div className="flex items-center justify-center my-[3rem]">
        <FindjobPagination
          totalItems={unsuitabledata.length}
